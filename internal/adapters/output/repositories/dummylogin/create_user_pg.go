...
func (r *PostgresRepo) DummyLogin(usertype valueobjects.UserType, userid string) (entities.User, error) {
	// Начало транзакции
	tx, err := r.db.Begin()
	if err != nil {
	...
	err = tx.QueryRow(`
        INSERT INTO users (user_id, usertype) 
        VALUES ($1, $2) 
        RETURNING user_id, usertype`,
		userid, usertype).Scan(&newUser.UserID, &newUser.Type)
	if err != nil {
